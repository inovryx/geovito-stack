...
SCRIPT_IN_CONTAINER="scripts/community_settings_smoke.js"
...
ensure_strapi_runtime() {
  ...
  if docker compose exec -T strapi sh -lc "test -f ${SCRIPT_IN_CONTAINER}" >/dev/null 2>&1; then
    ...
    host_hash="$(sha256sum "${ROOT_DIR}/app/scripts/community_settings_smoke.js" | cut -d ' ' -f1)"
    ...
    echo "INFO: Strapi container community settings smoke script guncel degil, rebuild yapiliyor..."
    docker compose up -d --build strapi >/dev/null
    return 0
  fi

  echo "INFO: Strapi container community settings smoke script bulunamadi, rebuild yapiliyor..."
  docker compose up -d --build strapi >/dev/null
}

echo "=============================================================="
echo "GEOVITO COMMUNITY SETTINGS SMOKE"
